#' @title ncaa_bb_games
#'
#' @description Obtains basketball game data by date from the ncaa stats API. Some notes:
#'
...
#' @param type Men's or Women's game schedule (options: 'men', 'women')
#'
...
#' }
#' @export
ncaa_bb_games <- function(date = Sys.Date(), type = 'men') {
    type |>
      (\(x) {
        if (length(x) != 1) {
          cli::cli_abort(
